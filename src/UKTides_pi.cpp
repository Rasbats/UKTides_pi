...
UKTides_pi::UKTides_pi(void *ppimgr)
  ...
  if (icon_path.type == IconPath::Type::Svg)
    ...
    wxLogWarning("Cannot find icon for basename: %s", "UKTides_panel_icon");
  ...
  if (m_show_UKTides_icon) {
    if (icon.type == IconPath::Type::Svg)
      m_leftclick_tool_id = InsertPlugInToolSVG(
          ...
          "UKTides", "", nullptr, UKTIDES_TOOL_POSITION, 0, this);
...
int UKTides_pi::GetAPIVersionMajor() { return atoi(API_VERSION); }
...
int UKTides_pi::GetPlugInVersionMajor() { return PLUGIN_VERSION_MAJOR; }

int UKTides_pi::GetPlugInVersionMinor() { return PLUGIN_VERSION_MINOR; }
...
wxBitmap *UKTides_pi::GetPlugInBitmap() { return &m_panel_bitmap; }

wxString UKTides_pi::GetCommonName() { return _("UKTides"); }

wxString UKTides_pi::GetShortDescription() { return _("UKTides"); }

wxString UKTides_pi::GetLongDescription() {
  return _("Downloads UKHO Tidal Data for UK ports");
}

int UKTides_pi::GetToolbarToolCount(void) { return 1; }
...
bool UKTides_pi::LoadConfig(void) {
  ...
  if (pConf) {
    if (pConf->HasGroup(_T("/Settings/UKTides_pi"))) {
      // Read the existing settings

      pConf->SetPath("/Settings/UKTides_pi");
      pConf->Read("ShowUKTidesIcon", &m_show_UKTides_icon, true);

      m_route_dialog_x = pConf->Read("DialogPosX", 20L);
      m_route_dialog_y = pConf->Read("DialogPosY", 20L);

      pConf->DeleteGroup(_T("/Settings/UKTides_pi"));
    } else {
      pConf->SetPath("/PlugIns/UKTides_pi");
      pConf->Read("ShowUKTidesIcon", &m_show_UKTides_icon, true);
